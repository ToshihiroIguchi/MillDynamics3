...
| `slurry.wettability` | Wettability | — | 0.6 | 0–1 | 0.05 | Ball<->slurry wettability, in [0, 1]. `0` = non-wetting (fluid is only ever pushed off a ball's surface, never pulled toward it — a 180° contact angle). `1` = strongly wetting. Not a true Young's-equation contact angle (no matching fluid-fluid cohesion term is implemented); only controls whether fluid clings to media at all. Adhesion is gated by each nearby fluid particle's own local density (`adhesion_wetness`), so it has essentially no effect on a ball in the air with no real slurry nearby — only on a ball actually at or under a slurry surface. See `docs/PHYSICS.md` §6.1a. |
